...
use QianXiong\Internal\Resource\Constants;
use QianXiong\Internal\Resource\V2Constants;
use QianXiong\Internal\Signature\DefaultSignature;
use QianXiong\Internal\Signature\V4Signature;
use QianXiong\Log\ObsLog;
...
trait SendRequestTrait
{
    ...
    public function __call($originMethod, $args)
    {
        ...
        if (!$operation) {
            ObsLog::commonLog(LogLevel::WARNING, 'unknow method ' . $originMethod);
        ...
        if (!$async) {
            ObsLog::commonLog(LogLevel::INFO, 'enter method ' . $originMethod . '...');
            $model = new Model();
            $model['method'] = $method;
            $params = empty($args) ? [] : $args[0];
            $this->checkMimeType($method, $params);
            $this->doRequest($model, $operation, $params);
            ObsLog::commonLog(LogLevel::INFO, 'obsclient cost ' . round(microtime(true) - $start, 3) * 1000 . ' ms to execute ' . $originMethod);
            unset($model['method']);
            return $model;
        } else {
            if (empty($args) || !(is_callable($callback = $args[count($args) - 1]))) {
                ObsLog::commonLog(LogLevel::WARNING, 'async method ' . $originMethod . ' must pass a CallbackInterface as param');
    ...
    protected function makeRequest($model, &$operation, $params, $endpoint = null)
    {
        ...
        ObsLog::commonLog(LogLevel::DEBUG, 'perform ' . strtolower($httpMethod) . ' request with url ' . $authResult['requestUrl']);
        ObsLog::commonLog(LogLevel::DEBUG, 'cannonicalRequest:' . $authResult['cannonicalRequest']);
        ObsLog::commonLog(LogLevel::DEBUG, 'request headers ' . var_export($authResult['headers'], true));
    ...
    protected function sendRequest($model, &$operation, $params, $request, $requestCount = 1)
    {
        ...
        $promise = $this->httpClient->sendAsync($request, ['stream' => $saveAsStream])->then(
            function (Response $response) use ($model, $operation, $params, $request, $requestCount, $start) {
                ...
                ObsLog::commonLog(LogLevel::INFO, 'http request cost ' . round(microtime(true) - $start, 3) * 1000 . ' ms');
            ...
            function ($exception) use ($model, $operation, $params, $request, $requestCount, $start) {
                ...
                if ($exception instanceof ConnectException) {
                    if ($requestCount <= $this->maxRetryCount) {
                        ...
                        ObsLog::commonLog(LogLevel::ERROR, $message);
                    }
    ...
    protected function sendRequestAsync($model, &$operation, $params, $callback, $startAsync, $originMethod, $request, $requestCount = 1)
    {
        ...
        return $this->httpClient->sendAsync($request, ['stream' => $saveAsStream])->then(
            function (Response $response) use ($model, $operation, $params, $callback, $startAsync, $originMethod, $request, $start) {
                ...
                $this->parseResponse($model, $request, $response, $operation);
                ObsLog::commonLog(LogLevel::INFO, 'obsclient cost ' . round(microtime(true) - $startAsync, 3) * 1000 . ' ms to execute ' . $originMethod);
                unset($model['method']);
                $callback(null, $model);
            },
