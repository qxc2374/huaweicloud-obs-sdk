...
use QianXiong\Internal\Common\CheckoutStream;
        ...
        if (is_string($filePath) && $filePath !== '') {
            ...
            try {
                ...
                if ($fp = fopen($filePath, 'w')) {
                    ...
                    fflush($fp);
                    ObsLog::commonLog(LogLevel::DEBUG, "write file %s ok", $filePath);
                } else {
                    ObsLog::commonLog(LogLevel::ERROR, "open file error,file path:%s", $filePath);
                }
            } finally {
                if ($fp) {
                    fclose($fp);
        ...
        try {
            ...
            if ($jsonErrorBody) {
                ...
                if ($data && is_array($data)) {
                    ...
                    ObsLog::commonLog(LogLevel::ERROR, "response body %s, and jsonToArray data is %s", $body, $data);
        ...
        try {
            ...
            if ($jsonErrorBody) {
                ...
                if ($jsonArray && is_array($jsonArray)) {
                    if (isset($jsonArray['request_id']) && $jsonArray['request_id'] !== '') {
                        $model['RequestId'] = strval($jsonArray['request_id']);
                    }
                    if (isset($jsonArray['code']) && $jsonArray['code'] !== '') {
                        $model['Code'] = strval($jsonArray['code']);
                    }
                    if (isset($jsonArray['message']) && $jsonArray['message'] !== '') {
                        $model['Message'] = strval($jsonArray['message']);
                    }
                } elseif ($jsonArray && strlen($jsonArray)) {
                    ObsLog::commonLog(LogLevel::ERROR, "response body %s, and jsonToArray data is %s", $body, $jsonArray);
