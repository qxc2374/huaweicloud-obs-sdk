...
class ObsClient
{
    ...
    public function __construct(array $config = [])
    {
        if (!isset($config['key'], $config['secret'])) {
            throw new \InvalidArgumentException('key and secret must be provided');
        }
        ...
        if ($endpoint === false || empty($endpoint['host'])) {
            throw new \InvalidArgumentException('endpoint must be a valid HTTP or HTTPS URL');
        }

        if (isset($config['logger'])) {
            if (!$config['logger'] instanceof LoggerInterface) {
                throw new ObsException('logger must implement Psr\\Log\\LoggerInterface');
            }
            ObsLog::setLogger($config['logger']);
        } elseif (isset($config['logConfig'])) {
            if (!is_array($config['logConfig'])) {
                throw new \InvalidArgumentException('logConfig must be an array');
            }
            ObsLog::initLog($config['logConfig']);
        }

        if (isset($config['signature'])) {
            $signature = strtolower(strval($config['signature']));
            if (!in_array($signature, [self::SigantureObs, self::SignatureV2, self::SignatureV4], true)) {
                throw new \InvalidArgumentException('signature must be one of: obs, v2, v4');
            }
        ...
        $this->httpClient = self::createHttpClient($config['handler'] ?? null);
    }

    private function createHttpClient($handler)
    {
        $options = [
            'timeout' => $this->timeout > 0 ? $this->timeout : 0,
        ...
        if (defined('CURLOPT_BUFFERSIZE')) {
            $options['curl'] = [CURLOPT_BUFFERSIZE => $this->chunkSize];
        }
    ...
    public function initLog(array $logConfig = [])
    {
        ...
        ObsLog::commonLog(\Psr\Log\LogLevel::WARNING, implode("];[", $msg));
    }
